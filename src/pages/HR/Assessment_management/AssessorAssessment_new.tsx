    ...
    const loadEmployeeAssessments = async () => {
      ...
      try {
        ...
        const { data, error: assessmentsError } = await supabase
          ...
          `)
          .eq('status', 'completed')
          .eq('assessor_status', 'reviewed') // Only show assessments reviewed by assessors
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-8 text-center">
              <div className="flex flex-col items-center justify-center space-y-4">
                ...
                <p className="text-gray-600 dark:text-gray-400 max-w-lg">
                  {searchTerm ? 'No assessments match your search criteria.' : 'There are no assessments that have been reviewed by assessors yet.'}
                </p>
              </div>
            </div>
          ) : (
